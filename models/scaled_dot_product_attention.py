...
class ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, query_key_size: int, *, temperature: Optional[float] = None) -> None:
        super().__init__()
        ...
        self.scale = torch.sqrt(torch.FloatTensor([query_key_size]))

        if temperature is not None:
            self.scale = torch.FloatTensor([temperature])
